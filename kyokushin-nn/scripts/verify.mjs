...
const allowedSourceHosts = new Set(["masterskayakarate.ru", "shin-nnov.orgs.biz", "disk.yandex.ru", "vk.com"]);
...
for (const item of realPhotos) {
  ...
  if (!isApprovedImageHost(imageHost)) throw new Error(`Gallery image must be a federation/VK photo, got: ${imageHost}`);
...
for (const [name, item] of Object.entries(registry.trainerPhotos ?? {})) {
  ...
  if (!item.image || !isApprovedImageHost(new URL(item.image).hostname)) throw new Error(`Trainer portrait must use an approved federation/VK image: ${name}`);
...
if (personPhotoCount < 1) throw new Error("At least one source-bound trainer portrait is expected");
...
console.log(
  `verify: gallery reliability PASS — 18 venues, 11 instructors, 8 real gallery photos, ` +
  `no visible dead-image tiles, 1 shared Masterskaya story, ${personPhotoCount} source-bound trainer portrait(s), 4 reports`
);
