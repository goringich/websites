...
if (registry.verifiedMedia.length < 6) {
  throw new Error(`Expected at least 6 verified club media items, got ${registry.verifiedMedia.length}`);
}

if (registry.photoCollections.length < 5) {
  throw new Error(`Expected at least 5 trusted photo collections, got ${registry.photoCollections.length}`);
}
...
console.log(
  `verify: 18 venues, 11 instructors, ${registry.verifiedMedia.length} verified media, ` +
  `${registry.photoCollections.length} trusted collections, identity guard PASS`
);
