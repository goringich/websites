...
- `art-direction-v2.css`, `experience.css`, `experience.js` — legacy/rejected visual sources retained for history/source compatibility only; the pure-V3 production build must strip them and release/live gates must fail if they return.
...
- `scripts/build-static.mjs` — canonical pure-V3 self-contained production bundle plus `dist/health.json` release/art-direction identity.
- `scripts/release-contract.mjs` — artifact-level release gate, including rejection of leaked V2 visual bundles.
- `scripts/production-smoke.mjs` — public exact-release/art-direction/pure-V3 runtime gate.
...
A major redesign after explicit rejection must also retire the rejected visual family from active production dependencies. A new art-direction stylesheet layered over rejected V2 CSS/JS is not sufficient. The current pure-V3 build intentionally strips `art-direction-v2.css`, `experience.css` and `experience.js` from `dist/index.html`; release and live smoke must fail if those bundles return.
...
`smoke:production` is a live check against `https://kyokushin-nn.vercel.app/` and fails closed when production is stale, on the wrong art direction, contains rejected V2 visual bundles, is bootstrapped through binary chunks, misses the required UI markers, has a missing/mismatching `/health.json`, or is unavailable.
...
For production work, the public domain must return the real pure-V3 self-contained site directly, HTML and `/health.json` must agree on the exact expected release and art direction, rejected V2 visual bundles must be absent, and the live smoke must pass; CI success alone is not a substitute.
